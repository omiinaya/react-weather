...
function demonstrateIssue() {
  ...
  console.log(
    ...
    utcMidnight.toLocaleDateString("en-US", { weekday: "long" }),
  );
  console.log(
    "Local midnight:",
    localMidnight.toISOString(),
    localMidnight.toLocaleDateString("en-US", { weekday: "long" }),
  );

  // Check if the issue is in the historical data fallback
  console.log("\n=== HISTORICAL DATA FALLBACK ANALYSIS ===");
  console.log(
    "In weather-history-utils.ts, when no forecast data is available,",
  );
  console.log(
    "fallback data is generated with arbitrary dates and temperatures.",
  );
...
console.log(
  "or if historical data is used as fallback, the day names might mismatch.",
);
