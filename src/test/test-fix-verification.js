...
function testDateAlignment(forecastDays) {
  ...
  forecastDays.slice(0, 3).forEach((day, index) => {
    ...
    console.log(
      `  ${formatted} - ${oldLabel} (WRONG: should be ${date.toLocaleDateString("en-US", { weekday: "long" })})`,
    );
  });
  ...
  forecastDays.slice(0, 5).forEach((day) => {
    ...
    const diffDays = Math.floor(
      (date.getTime() - new Date(todayStr).getTime()) / (1000 * 60 * 60 * 24),
    );
    ...
    console.log(
      `  ${formatted} - ${label} (${date.toLocaleDateString("en-US", { weekday: "long" })})`,
    );
  });
}
