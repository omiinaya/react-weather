...
function testDateCalculation() {
  ...
  console.log(
    "Target day:",
    actualToday.toLocaleDateString("en-US", { weekday: "long" }),
  );
  ...
  scenarios.forEach(({ name, date }) => {
    ...
    const diff = Math.floor(
      (date.getTime() - actualToday.getTime()) / (1000 * 60 * 60 * 24),
    );
  ...
  const diffDays = Math.floor(
    (apiDate.getTime() - new Date(todayStr).getTime()) / (1000 * 60 * 60 * 24),
  );
  console.log(
    "API date (2025-09-15):",
    apiDate.toLocaleDateString("en-US", { weekday: "long" }),
  );
  console.log("Calculated diffDays:", diffDays);
  console.log(
    "Expected label:",
    diffDays === 0 ? "Today" : diffDays === -1 ? "Yesterday" : "Other",
  );
}
...
console.log("\n=== SOLUTION IDENTIFIED ===");
console.log(
  "The issue is likely that the API returns dates starting from Sep 15,",
);
console.log(
  'but the calculation needs to ensure Sep 16 is identified as "Today".',
);
console.log(
  "We need to adjust the calculation to ensure the correct date is labeled as Today.",
);
