...
function simulateDateCalculations() {
  ...
  console.log(
    "Mock day of week:",
    mockToday.toLocaleDateString("en-US", { weekday: "long" }),
  );
  ...
  labels.forEach((item) => {
    console.log(
      `${item.dateStr}: ${item.label} (${item.dayOfWeek}) - diffDays: ${item.diffDays}`,
    );
  });
  ...
  console.log(
    "UTC day:",
    utcDate.toLocaleDateString("en-US", { weekday: "long" }),
  );
  console.log(
    "Local day:",
