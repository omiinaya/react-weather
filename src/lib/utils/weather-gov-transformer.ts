...
function getConditionCode(text: string): number {
  ...
  if (lowerText.includes("overcast") || lowerText.includes("mostly cloudy"))
    return 1009;
  if (lowerText.includes("cloudy") || lowerText.includes("mostly clear"))
    return 1006;
  if (
    lowerText.includes("partly") ||
    lowerText.includes("partly sunny") ||
    lowerText.includes("partly cloudy")
  )
    return 1003;
  if (
...
function parseWindSpeed(windSpeedStr: string): {
  mph: number;
  kph: number;
  direction: string;
} {
...
export class WeatherGovTransformer {
  static transformPointToLocation(
    point: WeatherGovPoint,
    lat: number,
    lon: number,
  ): CurrentWeatherResponse["location"] {
  ...
  static transformObservationToCurrentWeather(
    ...
    if (observation && props) {
      ...
      iconUrl =
        props.icon ||
        `https://api.weather.gov/icons/land/${isDay ? "day" : "night"}/skc?size=medium`;
      ...
      throw new Error(
        "Weather data unavailable - no observation or forecast data provided",
      );
    }
    ...
    const windSpeedKmh =
      ...
      parseWindSpeed(currentPeriod?.windSpeed || "5 mph").kph;
    ...
    const lastUpdated =
      props?.timestamp || (currentPeriod?.startTime ?? now.toISOString());
    ...
    return {
      ...
      precip_in: props?.textDescription?.toLowerCase().includes("rain")
        ? 0.04
        : 0,
      ...
      vis_miles:
        Math.round(((props?.visibility?.value ?? 10) / 1.60934) * 10) / 10,
  ...
  static transformForecastToResponse(
    ...
    const current = this.transformObservationToCurrentWeather(
      observation,
      location,
      forecast,
    );

    const forecastDays = this.transformForecastPeriods(
  ...
  static transformForecastPeriods(
    ...
    const days: Map<
      string,
      {
        date: string;
        date_epoch: number;
        daytime: WeatherGovForecastPeriod | null;
        nighttime: WeatherGovForecastPeriod | null;
      }
    > = new Map();
    ...
    if (periods.length > 0) {
      ...
      console.log(
        ...
        periods[periods.length - 1].startTime,
      );
    }
    ...
    const forecastDays = Array.from(days.values())
      .map(({ date, date_epoch, daytime, nighttime }) => {
        ...
        // Debug
        console.log(
          `Transformed day ${date}: day=${daytime?.temperature}, night=${nighttime?.temperature}`,
        );
        ...
        const { mph: windMph, kph: windKph } = parseWindSpeed(
          dayPeriod?.windSpeed ?? "5 mph",
        );
        ...
        return {
          ...
          day: {
            ...
            totalprecip_in:
              pop > 0 ? Math.round(((pop / 100) * 10) / 100 + 0.399) : 0,
            totalsnow_cm: 0,
