...
export const useWeatherHistoryCache = () => {
  ...
  const getFiveDayWindowDates = useCallback(
    (referenceDate: Date = new Date()): string[] => {
      const dates: string[] = [];
  ...
  const hasHistoricalData = useCallback(
    (location: string, date: string): boolean => {
  ...
  const getHistoricalData = useCallback(
    (location: string, date: string): ForecastDay | null => {
  ...
  const fetchHistoricalWeather = useCallback(
    async (location: string, date: string): Promise<ForecastDay | null> => {
